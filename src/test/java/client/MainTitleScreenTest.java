...
import client.GuiDecision.DecisionConnectionPoint.DecisionConnectionPoint;
...
public class MainTitleScreenTest {
    ...
    void try_opening_editor(FxRobot robot) {
        ...
        robot.drag(robot.lookup("#right-decision-connector").queryAs(DecisionConnectionPoint.class)).dropTo("#3");
        ...
        robot.clickOn("Fast");
        robot.clickOn(" Inventory");

        robot.closeCurrentWindow();
        robot.clickOn(" I am a new decision Test");
        robot.clickOn(" Replay the game?");
        robot.closeCurrentWindow();
